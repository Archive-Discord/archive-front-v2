...
const UserPage: NextPage<UserPageProps> = ({user, error, message, statusCode}) => {
  ...
  return (
      <>
        <HeadInfo title={user.username + " 프로필 - 아카이브"} image={userAvaterLink(user)} description={`${user.servers.map(x => x.name).join(', ')} 서버를 운영하고 ${user.bots.map(x => x.name).join(', ')} 봇을 제작합니다.`}/>
        ...
        <div className='flex lg:flex-row lg:justify-between lg:flex-nowarp flex-col items-center justify-center flex-warp mx-4'>
          ...
          <div className='mx-4'>
            ...
            <hr className='my-2'/>
            <div className='my-14 min-h-[10vh] flex flex-row flex-wrap justify-center'>
              {user.bots.length === 0 ? (<>
