...
import {
  ...
  guildProfileLink,
  userAvaterLink,
  userAvaterLinkAsPending,
} from "@utils/Tools";
import type { GetServerSideProps, NextPage } from "next";
import styles from "../../../styles/Server.module.css";
import DenyModal, {
  denyModalcallBackType,
} from "@components/pendinglist/denyModal";
...
export const getServerSideProps: GetServerSideProps = async context => {
  let bot = (await fetch(
    `${process.env.API_DOMAIN}/bots/${encodeURI(context.params.id as string)}`
...
const PendingBot: NextPage<botProps> = ({
  bot,
  error,
  statusCode,
  message,
  ...
  useEffect(() => {
    try {
      ...
      axios.get("/users/@me").then(data => {
        ...
        return setUser(
          JSON.parse(
            (localStorage.userData = JSON.stringify(data.data.data.user))
          )
        );
      });
    } catch (e) {
    ...
    <div className={styles.container}>
      ...
      <div className="flex lg:flex-row lg:justify-between lg:flex-nowarp flex-col items-center justify-center flex-warp mx-4">
        <div className="flex lg:flex-row flex-col items-center">
          <img
            className="lg:w-40 w-36 rounded-3xl"
            src={userAvaterLinkAsPending(bot)}
          />
          <div className="flex flex-col lg:items-start items-center">
            {bot.new ? null : (
              <>
                <span className="lg:ml-6 text-xl font-bold truncate lg:mt-0 mt-2 text-red-400">
                  <i className="fas fa-exclamation-triangle mr-2" />
                  정보 갱신불가
                </span>
              </>
            )}
            <span className="lg:ml-6 text-3xl font-bold truncate lg:mt-0 mt-1">
              {bot.name}
            </span>
            <span className="flex flex-row lg:ml-6 text-lg font-semibold lg:mt-2 mx-auto">
              <span className="mr-1">
                <i className="fas fa-server mr-1" />
                {formatNumber(bot.servers)}개
              </span>
              <span className="mx-2">
                <i className="fas fa-calendar-alt mr-1" />
                {DateFormet(bot.created_at).fromNow(true)}전 생성
              </span>
            </span>
          </div>
        </div>
        <div className="flex flex-row items-center text-xl lg:mt-0 mt-5 flex-wrap justify-center">
          <button
            onClick={() => openInviite(bot.invite)}
            className="lg:mt-0 mt-2 border bg-sky-500 px-5 rounded-xl text-white py-2 mx-1 hover:bg-sky-400 transform hover:-translate-y-1 transition duration-100 ease-in cursor-pointer"
          >
            초대하기
          </button>
          <Link href={`/bots/${bot.id}/like`}>
            <a className="lg:mt-0 border px-5 rounded-xl py-2 mx-1 hover:bg-stone-200 hover:-translate-y-1 transition duration-100 ease-in cursor-pointer mt-2">
              <i className="fas fa-caret-up mr-2" />
              좋아요 ({formatNumber(bot.like)})
            </a>
          </Link>
          {user ? (
            <>
              {bot.owners.filter(el => {
                ...
                <>
                  <Link href={`/bots/${bot.id}/edit`}>
                    <a className="lg:mt-0 flex items-center border px-5 rounded-xl py-2 mx-1 hover:bg-stone-200 hover:-translate-y-1 transition duration-100 ease-in cursor-pointer mt-2">
                          ...
                          <div className="flex items-center px-4 py-2">
                            <a
                              className={classNames(
                                active ? "bg-gray-100" : "",
                                "block text-rose-600 hover:text-sky-500 hover:underline hover:underline-offset-4"
                              )}
                            >
                              신고
                            </a>
        ...
        <div className="mb-1 w-full lg:w-1/4">
          <div className="py-3 px-7 text-black rounded my-4">
            <div className="flex flex-col">
              ...
              <div className="flex flex-row justify-between p-1">
                <span>
                  <i className="fas fa-calendar-alt mr-1" />봇 생성일
                </span>
              ...
              {bot.owners
                ...
                .map((owner, index) => (
                  <Link key={index} href={`/users/${owner.id}`}>
                    <a
                      target="_blank"
                      className="flex flex-row items-center p-2 rounded-3xl my-1 hover:bg-stone-200"
                    >
                      <img
                        className="w-10 rounded-full"
                        src={userAvaterLink(owner)}
                      />
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex flex-row flex-wrap">
                    {bot.support ? (
                      <>
                        <a
                          href={`https://discord.gg/${bot.support}`}
                          rel="noreferrer"
                          target="_blank"
                          className="border flex flex-row flex-wrap items-center rounded-xl my-1 hover:bg-stone-200 mx-1"
                        >
                          <span className="text-base p-1 px-2">
                            서포트 서버
                          </span>
                        </a>
                      </>
